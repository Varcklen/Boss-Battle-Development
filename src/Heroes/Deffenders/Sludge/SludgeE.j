scope SludgeE initializer init
	...
	globals
		private constant integer DURATION = 4
		
		private constant integer DAMAGE_INITIAL = 5
		private constant integer DAMAGE_PER_LEVEL = 3
	endglobals
	...
	private function SludgeECast takes nothing returns nothing
		...
		if GetUnitAbilityLevel( target, 'A0S2') == 0 then
			...
			call SaveUnitHandle( udg_hash, id1, StringHash( "slge1" ), target )
		    	...
		    	call SaveReal( udg_hash, id1, StringHash( "slge1" ), damage )
